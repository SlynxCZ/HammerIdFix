...
bool CGameConfig::IsSymbol(const char* name)
{
    ...
    {
        CORE_ERROR("Missing signature or symbol: {}\n", name);
        return false;
    }
...
void* CGameConfig::ResolveSignature(const char* name)
{
    ...
    else
    {
        ...
        if (!signature)
        {
            CORE_ERROR("Failed to find signature for {}\n", name);
            return nullptr;
        }
    ...
    if (!address)
    {
        CORE_ERROR("Failed to find address for {}\n", name);
        return nullptr;
    }
    return address;
}
