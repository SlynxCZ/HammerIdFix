...
namespace InventoryManager::modules {
    void Initialize() {
    ...
    std::replace(name.begin(), name.end(), '\\', '/');
        ...
        if (it == moduleList.end()) {
            InventoryManager_ERROR("Cannot find module {}", name);
...
CModule::CModule(std::string_view path, std::uint64_t base)
{
    ...
    if (should_read_from_disk)
    {
        ...
        if (!stream.good())
        {
            InventoryManager_ERROR("Cannot open file {}", m_pszPath);
            return;
        }
    ...
    for (auto i = 0; i < nt_header->FileHeader.NumberOfSections; i++, section++)
    {
        ...
        if (is_executable && is_readable)
        {
            ...
            if (should_read_from_disk)
            {
                ...
                {
                    InventoryManager_INFO("Copying bytes from disk for {}", m_pszPath);
                    segment.bytes = bytes.value();
                    continue;
                }
                InventoryManager_ERROR("Cannot get original bytes for {}", m_pszPath);
                return;
            }
    ...
    CModule::CModule(std::string_view path, dl_phdr_info *info) {
        ...
        const bool should_read_from_disk = std::any_of(modules_to_read_from_disk.begin(),
                                                       modules_to_read_from_disk.end(), [&](const auto &i) {
                                                           return m_pszModule == i;
                                                       });
...
void CModule::DumpSymbols()
{
    ...
    void *CModule::FindSignature(const char *signature) {
        ...
        {
            InventoryManager_ERROR("Cannot convert signture \"{}\" to bytes", signature);
            return nullptr;
        }
    ...
    void *CModule::FindInterface(std::string_view name) {
        if (_interfaces.empty()) {
            ...
            if (ret_interface == nullptr) {
                ...
                InventoryManager_ERROR("Could not find interface {} in {}", name, m_pszModule);
                Plat_ExitProcess(1);
            }
    ...
    void *CModule::FindSymbol(const std::string &name) {
        ...
        InventoryManager_ERROR("Cannot find symbol {}", name);
        return nullptr;
    }
} // namespace InventoryManager::modules
