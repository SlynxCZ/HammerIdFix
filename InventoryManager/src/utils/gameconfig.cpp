...
const char* CGameConfig::GetSymbol(const char* name)
{
    ...
    {
        InventoryManager_ERROR("Missing symbol: {}\n", name);
        return nullptr;
    }
    return symbol + 1;
}
...
{
    InventoryManager_ERROR("Not implemented.");
    return nullptr;
}
...
bool CGameConfig::IsSymbol(const char* name)
{
    ...
    {
        InventoryManager_ERROR("Missing signature or symbol: {}\n", name);
        return false;
    }
...
void* CGameConfig::ResolveSignature(const char* name)
{
    ...
    if (!module || !(*module))
    {
        InventoryManager_ERROR("Invalid Module {}\n", name);
        return nullptr;
    }
    ...
    if (this->IsSymbol(name))
    {
        ...
        if (!symbol)
        {
            InventoryManager_ERROR("Invalid symbol for {}\n", name);
            return nullptr;
        }
    ...
    else
    {
        ...
        if (!signature)
        {
            InventoryManager_ERROR("Failed to find signature for {}\n", name);
            return nullptr;
        }
    ...
    if (!address)
    {
        InventoryManager_ERROR("Failed to find address for {}\n", name);
        return nullptr;
    }
    return address;
}
